...
const FLASH_KEY: &str = "solitaire-flash";
const SESSION_ID_KEY: &str = "solitaire-session";

/// Creates a [RequestContext](solitaire_service::RequestContext) from an HTTP request
pub fn req_context(session: &Session, cfg: &Data<AppConfig>, action_key: &'static str) -> RequestContext {
  ...
  let user_profile = solitaire_service::profile::load(app_cfg.files(), user_id);
  let logger = app_cfg
    .root_logger()
